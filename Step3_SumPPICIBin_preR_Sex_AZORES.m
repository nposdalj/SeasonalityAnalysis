...
% AZORES version - loops over both Azores sites, matching the flat F:\AZORES
% output layout (no per-site subfolder) used by Step1/Step2 AZORES versions.
...
dataDir = 'F:\AZORES\SeasonalityAnalysis'; % everything lives under F:\AZORES (matches the Step1/Step2 AZORES scripts)
saveDir = dataDir; %outputs for both sites are saved here, prefixed with siteabrev - no per-site subfolder
effortXls = fullfile(dataDir,'Pm_Effort.xlsx'); % one workbook, one sheet per site (sheet name = siteabrev)

%Sites to process - must match siteabrev used in the Step1/Step2 AZORES scripts
...
genderFileName = siteabrev; %File name to match gender file
...
saveDir = 'F:\AZORES\SeasonalityAnalysis'; % everything lives under F:\AZORES (matches the Step1/Step2 AZORES scripts)
filename = [saveDir,'\',siteList{iSite},'\',genderFileName,'_',sp,'_gender_filled.mat'];
load(filename);
sexData = binData_mod; %not to get in the way of other bin data
%% load workspace 2
load([saveDir,'\',siteList{iSite},'\',siteabrev,'_workspaceStep2.mat']);
