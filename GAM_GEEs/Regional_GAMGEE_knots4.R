...
AvgYearBasis <- gam(PreAbs~s(Year, bs ="cc", k=4), fit=F, data = SiteHourTableB, family =binomial, knots = list(HOUR=seq(2011,2019,length=4)))$X[,2:3]
AvgYearMat = as.matrix(AvgYearBasis)
...
#GOA
dimnames(AvgDayMat)<-list(NULL,c("ADBM1", "ADBM2"))
dimnames(AvgYearMat)<-list(NULL,c("AYBM1", "AYBM2"))
PODFinal = geeglm(PreAbs ~ AvgDayMat + as.factor(Year),family = binomial, corstr="ar1", id=Blocks, data=SiteHourTableB)
PODFinalY = geeglm(PreAbs ~ AvgDayMat + AvgYearMat ,family = binomial, corstr="ar1", id=Blocks, data=SiteHourTableB)
summary(PODFinalY)
# Call:
