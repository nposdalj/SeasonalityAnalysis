...
%% Loop through sites. Go find something fun to do, this takes a while :)
...
for site = 1:length(siteabrevs)
    ...
    GDrive = GDrive_real;       % Restore the correct GDrive :D
    dayBinCSV(1) = GDrive;
    effortXls(1) = GDrive;
    filename(1) = GDrive;
    tpwsPath(1) = GDrive;
    ...
    sexbinPresence_E.FemaleHoursNorm = (sexbinPresence_E.FemaleNormBin*5)./60; % CORRECT HOURS NORM!!!
    sexbinPresence_E.JuvenileHoursNorm = (sexbinPresence_E.JuvenileNormBin*5)./60; % CORRECT HOURS NORM!!!
    sexbinPresence_E.MaleHoursNorm = (sexbinPresence_E.MaleNormBin*5)./60; % CORRECT HOURS NORM!!!
...
mtFE365WEEK_BACKUPreg = mtFE365WEEK_reg; % Because generating them took a little while
...
presenceMax = max([max(mtGN365WEEK_reg(:)) max(mtFE365WEEK_reg(:)) max(mtJU365WEEK_reg(:)) max(mtMA365WEEK_reg(:))]);
    % Find absolute max across all plots so they can be set to the same color scale

for i = 1:4
    if i == 1       % Cycle through GN, FE, JU, and MA (generalized as XX) regional weekly meantabs and plot each
    ...
    mtXX365WEEK_regTab = [array2table((1:52).') array2table(mtGN365WEEK_reg)]; % Convert to table AND include column for week numbers
    ...
    mtXX365WEEK_regPlot(1:52,1:length(siteabrevs)) = mtXX365WEEK_reg; % Make version that can be plotted, since pcolor plots vertices
    % (and will therefore be missing one row and one column of values without this adjustment)
...
% figure(110)
% weekly_bars = bar3(mtGN365WEEK_reg);
% set(gca, 'XDir', 'reverse')
% set(gca, 'View', [-37.5000 45])
% set(weekly_bars, 'EdgeColor', 'w')
% for k = 1:length(weekly_bars)
%     zdat = weekly_bars(k).ZData;
%     weekly_bars(k).CData = zdat;
%     weekly_bars(k).FaceColor = 'interp';
% end