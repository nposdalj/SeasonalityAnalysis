...
% This script creates plots of weekly average presence of sperm whales at
% all sites, with the average number of hours with presence indicated by
% color.

%% Parameters defined by user
siteabrevs = {'HZ' 'OC' 'NC' 'BC' 'WC' 'GS' 'BP' 'BS' 'JAX'}; %abbreviations of sites.
...
saveDir = [GDrive,':\My Drive\',region,'_TPWS_metadataReduced\Plots\GENERAL']; % Where tables and plots are saved
...
meantab365WEEK_regionalBACKUP = mtGN365WEEK_reg;
BACKUPmeantab365WEEK_regiona = mtGN365WEEK_reg;

meantab365WEEK_regPlot = nan(53,length(siteabrevs)+1);
meantab365WEEK_regPlot(1:52,1:length(siteabrevs)) = mtGN365WEEK_reg; % Make version that can be plotted, since pcolor plots vertices
% (and will therefore be missing one row and one column of values)

%% Save avg weekly presence summary arrays, as tables
...
saveas(allsites_cplot, saveDir, 'png');
