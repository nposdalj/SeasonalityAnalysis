...
predgridmakerONE <- function (model1, thingstoshow,namethingstoshow, axisLabel=axisLabel, maxy=maxyVal, maxviolin=maxViolinVal){
  ...
  names (gridpred)[1] <- namethingstoshow
  #names (gridpred)[2:dim (gridpred)[2]]<- otherstuff
  gridpred$Pred <- predict (fit1, gridpred, type="response")
  bspreds <- matrix (NA, bootN, dim (gridpred)[1])
  bootstrapmodel <- model1
  set.seed (101)
  #############Bootstrapping to obtain confidence interval 
...
windows() ###### Plots jd
otherstuff = c("sPres","sLag","sProp","maxRLpp", "sunriseLag","sunsetLag", "year")
otherstuffvalue <- c(1,0,mean (zcdata$sProp), mean (zcdata$maxRLpp), 12, 12,2011)       ###perhaps better to set jd=183
namethingstoshow= "jd"
axisLabel = "Julian Date"
predgridmaker (zc.gee3, zcdata$jd, namethingstoshow, otherstuff, otherstuffvalue,axisLabel, maxy=0.015, maxviolin = 0.015)
